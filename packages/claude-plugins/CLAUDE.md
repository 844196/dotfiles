...
[`./plugins`](./plugins/) に "新しいプラグインを追加" "既存のプラグインをリネーム" "既存のプラグインを削除" したときは、必ず [ユーザーレベルの Claude Code 設定ファイル](../../files/dot_claude/settings.json.tmpl) の `enabledPlugins` と必要があれば `pluginSettings` を更新し `chezmoi apply` すること。
