...
`herdr` はすでに PATH に入っている。**コマンド構文の正はこのスキルではなくインストール済みバイナリ**。まず `herdr --help` でトップレベルの usage を確認し、次に対象のコマンドグループを引数無しで叩いてサブコマンド一覧・シグネチャを見る:

```bash
herdr --help          # トップレベル usage
herdr pane            # コマンドグループを引数無しで叩くとサブコマンド一覧が出る (herdr pane --help でも可)
herdr agent
herdr wait
```

discovery のつもりで **素の `herdr` を叩かない** — TUI を起動 / アタッチしてしまう。`create` のような mutating な leaf サブコマンドも「引数を見たいだけ」で引数無しで叩かない (デフォルト値でそのまま実行される。罠を参照)。生のソケット API プロトコルやフルリファレンスが必要なら [socket API docs](https://herdr.dev/docs/socket-api/) を読む。
...
`idle` と `done` は **同じ「作業していない」状態を、結果が見られたか (attention) で区別したもの** (以下の遷移は実機で確認済み):

- `idle` — 待機中で、結果は「もう見られた」扱い。
- `done` — 完了したが、結果がまだ見られていない。

`working` / `blocked` からの完了が `done` になるか `idle` になるかは、その pane の tab が見られているかで決まる:

- エージェントがプロンプトで起動した直後は、バックグラウンドの tab でも `idle` (実機で確認済み: 非フォーカスの tab で `claude` を起動したら `idle` になった)。
- 完了時にその tab がバックグラウンド (フォーカスされていない) なら `done`、フォアグラウンドで見られている active tab なら `idle` (実機で確認済み: 同じエージェントを背面で完了させると `done`、フォーカス中に完了させると `idle` になった)。
- `done` の pane の tab にフォーカスして「見られた」状態にすると、`done` は `idle` に落ちる (実機で確認済み: 背面完了で `done` になった pane を `tab focus` したら `idle` に遷移した)。

pane の完了を検知したいときは `idle` と `done` の **どちらも「完了」として扱う**。違いは結果が見られたかどうかだけで、ユーザーがその tab を見ているなら `idle`、バックグラウンドなら `done` になる (wait の使い分けは後述)。
...
### 引数を省いた mutating な leaf サブコマンドはデフォルト値で実行される

discovery のつもりでサブコマンドを引数無しで叩くと、mutating なコマンドはそのまま実行される。実機で確認済み: `herdr workspace create` はフラグが全て optional (`[--cwd PATH] [--label TEXT] [--env KEY=VALUE] [--focus] [--no-focus]`) なので、`herdr workspace create --no-focus` のように意味のある引数を渡さなくてもデフォルト値 (label は cwd のフォルダ名) で実際に新しい workspace を作る (作られた workspace を `workspace close` で片付けて確認した)。加えて leaf サブコマンドは `--help` を受け付けない (`herdr workspace create --help` は `unknown option`)。構文を見たいだけなら leaf を引数無しで叩くのではなく、コマンドグループを引数無しで叩く (`herdr workspace`) か `herdr <group> --help` を使う。素の `herdr` も discovery に使わない (TUI を起動 / アタッチしてしまう)。
