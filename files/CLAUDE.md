...
Chezmoi のソースルートディレクトリ。

Nested `CLAUDE.md` は [chezmoi の除外設定](.chezmoiignore) によって除外される。

## ルール

### ディレクトリには可能な限り `exact_` を付ける

付けないとソース上削除したファイルがホームディレクトリ以下に残り続けてしまうため、ディレクトリには `exact_` を付ける。
...
- `exact_` を付けない
  ...
  - e.g. [`dot_claude/`](dot_claude/CLAUDE.md)
