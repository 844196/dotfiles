...
| 検知対象 | 出るリマインダー |
|---|---|
| `.md` ファイルの内容変更・新規・削除 | `[doc-consistency]` — 章立ての全体俯瞰、内部矛盾・用語の揺れ・見出し階層・相互参照の整合性などのチェックリスト |
...
| Hook | Event | 動作 |
|---|---|---|
| [`session-start-gc.sh`][session-start-gc] | `SessionStart` | state root 配下の `<session_id>/` を sweep し、7 日以上更新されていない dir を削除 (終了済みセッションの掃除) |
| [`reset-turn-state.sh`][reset-turn-state] | `UserPromptSubmit` | 自セッションの state dir をクリア、リポジトリルートを `repo_root` に記録、`ls-files` × `git hash-object` でリポジトリ配下の `<hash>\t<path>` 一覧 (`snap-files`) を保存 |
| [`edit-reminders-on-stop.sh`][edit-reminders-on-stop] | `Stop` | スナップショットと現状の差分から `has_md` / `has_impl` / `has_deleted` を判定、reason を組み立て `decision: "block"` で注入。`blocked` フラグを立てて二重発火を防ぐ |
...
`git status` 系は `--no-optional-locks` 付きで実行する (`.git/index.lock` の残置回避、cf. [zen プロンプトのセットアップ](../../../exact_dot_zsh/exact_functions/exact_Prompts/prompt_zen_setup))。
...
- `.gitignore` で除外されたパス (e.g. [personal-scratchpad](../exact_personal-scratchpad/CLAUDE.md) が管理する `.844196/` などの作業メモ)
- git リポジトリでない CWD (snapshot を取る対象が無いため、Stop でも何もしない)
- 内容を変えない操作 (`git add` / `git commit` / 実行ビット変更のみ等)

設計判断: ノイズ (CWD 外への書き込み、[personal-scratchpad](../exact_personal-scratchpad/CLAUDE.md) が管理する `.844196/` への作業メモ等) を構造的に除外したいので、検知の根拠を CWD の git 作業ツリーに統一した。経路 (Edit/Write/Bash/Agent/MCP) によらずファイルシステム状態のみで判定するため、検知漏れも誤検知も少ない。

[session-start-gc]: exact_hooks/executable_session-start-gc.sh
[reset-turn-state]: exact_hooks/executable_reset-turn-state.sh
[edit-reminders-on-stop]: exact_hooks/executable_edit-reminders-on-stop.sh
