...
| `SessionStart` | (なし) | メインに対し、atuin 利用案内・[`command-history-analyzer`](exact_agents/command-history-analyzer.md) / [`session-analyzer`](../exact_session-analyzer/exact_agents/session-analyzer.md) との役割分担・直接叩く vs エージェント経由の使い分けを注入 |
...
メインエージェントが「メインで直接叩くか / エージェントに委ねるか」を判断できるよう、[セッション開始ルール]の本文を `SessionStart` で事前告知している:
