...
| `SessionStart` | (なし) | メインに対し、atuin 利用案内・[`command-history-analyzer`](exact_agents/command-history-analyzer.md) / [`session-analyzer`](../exact_session/exact_agents/session-analyzer.md) との役割分担・直接叩く vs エージェント経由の使い分けを `additionalContext` で注入 |
...
| [`session:session-analyzer`](../exact_session/exact_agents/session-analyzer.md) | 会話 transcript (なぜそれをやったか・どんな思考を経たか) | `~/.claude/projects/*/<session-id>.jsonl` |
