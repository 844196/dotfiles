...
配置先の `~/.claude/skills/` は chezmoi では管理しない (apm 専用領域)。自作スキル・エージェントは [ローカルマーケットプレイス](../dot_claude/exact_local-marketplace/) 経由のプラグインとして配置するため、apm と chezmoi の配置先は重ならない。
