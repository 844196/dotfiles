...
- `dot_claude/` — Claude Code が `projects/`、`todos/`、`sessions/`、`history.jsonl` 等を動的に生成
...
- **`symlink_`**: 特定のパスを要求するツールを実体ファイルにリダイレクトしたいときに使います。実例: `symlink_dot_zshenv` → 中身 `.zsh/.zshenv` で、`~/.zshenv` を `~/.zsh/.zshenv` の symlink にしています。
...
実例: `files/dot_claude/exact_skills/exact_commit/SKILL.md.tmpl` が `.chezmoitemplates/commit-skill-steps.md` を include。
...
- **zsh プラグイン** (`.zsh/plugins/*`): プラグインマネージャを使わずコミットハッシュ固定で取り込む
...
external 管理下のパス (例: `.zsh/plugins/*`) を `files/exact_dot_zsh/plugins/` 配下に直接置かないでください。親ディレクトリは `exact_` なしで残しておき、中身は external 展開に任せます。
