...
>
> - `~/.zshenv` (実体は `~/.config/zsh/.zshenv`、ソース: `files/symlink_dot_zshenv` + `files/dot_config/exact_zsh/dot_zshenv`)
> - `~/.config/zsh/.zshrc` (ソース: `files/dot_config/exact_zsh/dot_zshrc`)
...
> TODO: `autoload -Uz` 群 (compinit, select-word-style, smart-insert-last-word, {up,down}-line-or-beginning-search, zman, anyframe-init, ...) と、`$XDG_DATA_HOME/zsh/functions/**/*` の自作関数の関係。配置元 (external / apm / mise / packages / 自作) の対応も。
...
> TODO: `compinit` の起動位置、`$XDG_DATA_HOME/zsh/vendor-completions/` (external で配置), mise/apm/packages 経由の補完、`exact_` 判断と `.zcompdump` の扱い。
