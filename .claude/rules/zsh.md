...
zsh の挙動は `files/exact_dot_zsh/` だけでは完結せず、`packages/`、`files/exact_dot_844196/`、`files/.chezmoiexternal.yaml`、`files/.chezmoiscripts/`、`files/dot_apm/`、`files/dot_claude/` および ターゲット側の `~/.844196/`, `~/.zsh/`, `~/.cache/zsh/`, `~/.local/state/zsh/` 等の組み合わせ結果として成立する。片方だけ修正してもう片方が追従していない型のバグが起きやすい。
